;;; reference-explorer-lookup-homebrew.el --- Homebrew GNU Lookup setup -*- lexical-binding: t -*-

;;; Commentary:

;; Optional helper for a GNU Lookup installation supplied by Homebrew.  Load
;; and call `reference-explorer-lookup-homebrew-configure' before loading
;; `reference-explorer-lookup'.  The core lookup UI does not call it.
...
(defcustom reference-explorer-lookup-homebrew-prefix nil
...
(defun reference-explorer-lookup-homebrew--prefix ()
  "Return the configured or inferred Homebrew prefix."
  (or reference-explorer-lookup-homebrew-prefix
...
(defun reference-explorer-lookup-homebrew--program (prefix name)
...
;;;###autoload
(defun reference-explorer-lookup-homebrew-configure ()
  "Configure GNU Lookup, EBLook, and its MeCab backend from Homebrew.
Call this before loading `reference-explorer-lookup' so `lookup-kanji' sees
the selected backend during initialization."
  (interactive)
  (let* ((prefix (or (reference-explorer-lookup-homebrew--prefix)
    ...
    (when-let ((eblook
                (reference-explorer-lookup-homebrew--program prefix "eblook")))
      (setq ndeb-program-name eblook))
    (when-let ((mecab
                (reference-explorer-lookup-homebrew--program prefix "mecab")))
      (setq lookup-kanji-mecab-program-name mecab))))

(provide 'reference-explorer-lookup-homebrew)
;;; reference-explorer-lookup-homebrew.el ends here
