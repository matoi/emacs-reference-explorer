;;; reference-explorer-docset-test.el --- Tests for local docsets -*- lexical-binding: t -*-

(require 'ert)
(require 'reference-explorer-docset)

(defun reference-explorer-docset-test--create (directory name rows)
...
(defun reference-explorer-docset-test--create-zdash (directory name)
  "Create a minimal ZDash docset NAME under DIRECTORY."
  (let* ((root (reference-explorer-docset-test--create directory name nil))
         (database-file
...
(defun reference-explorer-docset-test--write-metadata (root feed version)
  "Record managed FEED and VERSION metadata inside docset ROOT."
  (let ((json-encoding-pretty-print t))
    (with-temp-file
        (expand-file-name reference-explorer-docset-metadata-file-name root)
      (insert (json-encode `((feed . ,feed) (version . ,version))) "\n"))))

(ert-deftest reference-explorer-docset-selects-installed-versions-by-selector ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-versioned-" t))
         (feed-directory (expand-file-name "Ruby" temporary))
         (reference-explorer-docset-directories (list temporary))
         (reference-explorer-docset--cache :uninitialized))
    (unwind-protect
        (progn
          (make-directory feed-directory)
          (dolist (version '("3.3.6" "3.3.7" "4.0.6"))
            (let ((root
                   (reference-explorer-docset-test--create
                    feed-directory (format "Ruby-%s" version)
                    `((,(format "Ruby-%s" version)
                       "Guide" "index.html")))))
              (reference-explorer-docset-test--write-metadata root "Ruby" version)))
          (dolist (case '(("Ruby" . "4.0.6")
                          ("Ruby@latest" . "4.0.6")
                          ("Ruby@3.3.*" . "3.3.7")
                          ("Ruby@3.3.6" . "3.3.6")))
            (let ((reference-explorer-docset-mode-alist
                   `(((ruby-mode) . (,(car case))))))
              (should
               (equal
                (reference-explorer-docset-version
                 (car (reference-explorer-docset-for-mode 'ruby-mode)))
                (cdr case))))))
      (delete-directory temporary t))))

(ert-deftest reference-explorer-docset-warns-once-for-missing-file-mode ()
  (let ((reference-explorer-docset-directories nil)
        (reference-explorer-docset-mode-alist
         '(((ruby-mode ruby-ts-mode) . ("Ruby@3.3.*"))))
        (reference-explorer-docset--cache :uninitialized)
        (reference-explorer-docset--warned-missing (make-hash-table :test #'equal))
        warnings)
    (cl-letf (((symbol-function 'display-warning)
      ...
      (with-temp-buffer
        ...
        (reference-explorer-docset-warn-if-missing)
        (reference-explorer-docset-warn-if-missing))
      (should (= (length warnings) 1))
      (should (eq (caar warnings) 'reference-explorer-docset))
      (should (string-match-p "Ruby@3.3.\\*" (cadar warnings)))
      (should (string-match-p "next provider" (cadar warnings))))))

(ert-deftest reference-explorer-docset-does-not-warn-when-selector-matches ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-warning-" t))
         (feed-directory (expand-file-name "Ruby" temporary))
         (reference-explorer-docset-directories (list temporary))
         (reference-explorer-docset-mode-alist
    ...
    (unwind-protect
        (progn
          (make-directory feed-directory)
          (let ((root (reference-explorer-docset-test--create
                       feed-directory "Ruby-4.0.6" nil)))
            (reference-explorer-docset-test--write-metadata root "Ruby" "4.0.6"))
          (cl-letf (((symbol-function 'display-warning)
...
(ert-deftest reference-explorer-docset-discovers-and-orders-search-results ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-test-" t))
         (reference-explorer-docset-directories (list temporary))
         (reference-explorer-docset-mode-alist
          '(((ruby-mode) . ("Ruby" "RubyExtra"))))
         (reference-explorer-docset-max-results 20)
         (reference-explorer-docset--cache :uninitialized))
    (unwind-protect
        (progn
          (reference-explorer-docset-test--create
          ...
          (let ((results (reference-explorer-docset-search "Array" 'ruby-mode)))
            (should (equal (mapcar #'reference-explorer-docset-result-name results)
                           '("Array" "Array" "ArrayLike" "Array methods")))
            (should
             (equal
              (mapcar (lambda (result)
                        (reference-explorer-docset-name
                         (reference-explorer-docset-result-docset result)))
                      results)
              '("Ruby" "RubyExtra" "Ruby" "RubyExtra"))))
          (should
           (equal (mapcar #'reference-explorer-docset-result-name
                          (reference-explorer-docset-search "A%" 'ruby-mode))
                  '("A%Literal")))
          (let ((results (reference-explorer-docset-search "require" 'ruby-mode)))
            (should
             (equal (mapcar #'reference-explorer-docset-result-name results)
                    '("Kernel.require" "Bundler.require"
                      "Required gemspec attributes")))
            (should
             (equal (mapcar #'reference-explorer-docset-result-exact results)
                    '(t t nil)))))
      (delete-directory temporary t))))

(ert-deftest reference-explorer-docset-renders-local-html-with-shr ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-render-" t))
         (root (reference-explorer-docset-test--create
                temporary "Ruby"
                '(("Array" "Class" "index.html#target"))))
         (docset (reference-explorer-docset--bundle root))
    ...
    (unwind-protect
        (progn
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-render-test*"))
          (with-current-buffer buffer
            (should (derived-mode-p 'reference-explorer-docset-mode))
            (should (equal reference-explorer-docset-current-result result))
...
(ert-deftest reference-explorer-docset-renders-dash-encoded-anchor-at-point ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-anchor-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
    ...
    (unwind-protect
        (progn
          ...
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-anchor-test*"))
          (with-current-buffer buffer
            (should (= (point) (point-min)))
...
(ert-deftest reference-explorer-docset-normalizes-dash-path-metadata ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-metadata-path-" t))
         (reference-explorer-docset-directories (list temporary))
         (reference-explorer-docset-mode-alist '(((ruby-mode) . ("Ruby"))))
         (reference-explorer-docset--cache :uninitialized))
    (unwind-protect
        (progn
          (reference-explorer-docset-test--create
          ...
          (let ((result (car (reference-explorer-docset-search
                              "Gem::Specification" 'ruby-mode))))
            (should result)
            (should (equal (reference-explorer-docset-result-path result)
                           "index.html#target"))
            (should (file-regular-p (reference-explorer-docset-result-file result)))))
      (delete-directory temporary t))))

(ert-deftest reference-explorer-docset-renders-only-one-dash-entry ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-section-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (documents (expand-file-name "Contents/Resources/Documents" root))
         (docset (reference-explorer-docset--bundle root))
    ...
    (unwind-protect
        (progn
          ...
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-section-test*"))
          (with-current-buffer buffer
...
(ert-deftest reference-explorer-docset-renders-rdoc-description-before-source ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-rdoc-order-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (documents (expand-file-name "Contents/Resources/Documents" root))
         (docset (reference-explorer-docset--bundle root))
         (fragment "//dash_ref_method-i-require/Method/require/0")
        ...
        (progn
          (with-temp-file (expand-file-name "index.html" documents)
            (insert "<html><body>"
                    ...
                    "</div></body></html>"))
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-rdoc-order-test*"))
          (with-current-buffer buffer
            (let* ((rendered (replace-regexp-in-string
                              "[[:space:]]+" " " (buffer-string)))
...
(ert-deftest reference-explorer-docset-builds-isolated-styled-html ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-html-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (documents (expand-file-name "Contents/Resources/Documents" root))
         (docset (reference-explorer-docset--bundle root))
         (fragment "//dash_ref_method-i-require/Method/require/0")
        ...
        (progn
          (with-temp-file (expand-file-name "index.html" documents)
            (insert "<html><head>"
                    ...
                    "</div></body></html>"))
          (let ((html (reference-explorer-docset-render-html
                       result "html { font-size: 14px; }")))
            (should (string-match-p "reference-explorer-docset-entry" html))
...
(ert-deftest reference-explorer-docset-renders-alias-target-content ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-alias-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (documents (expand-file-name "Contents/Resources/Documents" root))
         (docset (reference-explorer-docset--bundle root))
         (result (reference-explorer-docset-result-create
        ...
        (progn
          (with-temp-file (expand-file-name "Date.html" documents)
            (insert "<html><body>"
                    ...
                    "</body></html>"))
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-alias-test*"))
          (with-current-buffer buffer
            (let ((rendered (replace-regexp-in-string
                             "[[:space:]]+" " " (buffer-string))))
...
(ert-deftest reference-explorer-docset-no-fragment-starts-at-article-heading ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-article-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (documents (expand-file-name "Contents/Resources/Documents" root))
         (docset (reference-explorer-docset--bundle root))
         (result (reference-explorer-docset-result-create
        ...
        (progn
          (with-temp-file (expand-file-name "Array.html" documents)
            (insert "<html><body><main><ol role=\"navigation\">"
                    ...
                    "</main></body></html>"))
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-article-test*"))
          (with-current-buffer buffer
            (let ((rendered (replace-regexp-in-string
                             "[[:space:]]+" " " (buffer-string))))
...
(ert-deftest reference-explorer-docset-fontifies-classified-code-blocks ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-code-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (documents (expand-file-name "Contents/Resources/Documents" root))
         (docset (reference-explorer-docset--bundle root))
         (result (reference-explorer-docset-result-create
        ...
        (progn
          (with-temp-file (expand-file-name "example.html" documents)
            (insert "<html><body><main><h1>Example</h1>"
                    ...
                    "</main></body></html>"))
          (setq buffer (reference-explorer-docset-render
                        result " *reference-explorer-docset-code-test*"))
          (with-current-buffer buffer
            (goto-char (point-min))
            (search-forward "def")
            ...
            (should
             (memq 'reference-explorer-docset-code-block
                   (ensure-list
                    (get-text-property (match-beginning 0) 'face))))))
      (when (buffer-live-p buffer) (kill-buffer buffer))
      (delete-directory temporary t))))

(ert-deftest reference-explorer-docset-searches-zdash-schema ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-zdash-" t))
         (reference-explorer-docset-directories (list temporary))
         (reference-explorer-docset-mode-alist
    ...
    (unwind-protect
        (progn
          (reference-explorer-docset-test--create-zdash temporary "ZDash")
          (let ((result (car (reference-explorer-docset-search
                              "Array" 'emacs-lisp-mode))))
            (should (equal (reference-explorer-docset-result-name result) "Array"))
            (should (equal (reference-explorer-docset-result-path result)
                           "index.html#target"))))
      (delete-directory temporary t))))

(ert-deftest reference-explorer-docset-rejects-path-outside-documents ()
  (let* ((temporary (make-temp-file "reference-explorer-docset-safe-" t))
         (root (reference-explorer-docset-test--create temporary "Ruby" nil))
         (docset (reference-explorer-docset--bundle root))
         (result (reference-explorer-docset-result-create
                  ...
                  :docset docset)))
    (unwind-protect
        (should-not (reference-explorer-docset-result-file result))
      (delete-directory temporary t))))

(provide 'reference-explorer-docset-test)
;;; reference-explorer-docset-test.el ends here
